...
const TIME: &str = "2023-09-11T15:00:00.000Z";
...
pub async fn onexd_setup(
    ...
    //let ccvconsumer_state: Value =
    // serde_json::from_str(ccvconsumer_state_s).stack()?; genesis["app_state"
    // ]["ccvconsumer"] = ccvconsumer_state;
...
async fn consumer(args: &Args) -> Result<()> {
    ...
    let ccvconsumer_state_s: String = "".to_string(); //nm_onomyd.recv().await.stack()?;
